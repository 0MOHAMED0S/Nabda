...
<div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8" x-data="{
    ...
    <div class="flex flex-col justify-between items-start gap-4 mb-8 md:mb-10">
        <div>
            ...
            <p class="text-slate-500 mt-2 text-sm">إدارة الأهداف الاستراتيجية والقيم التي تسعى المنصة لتحقيقها.</p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-dark-800 p-6 md:p-8 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700/50 mb-10 w-full">
        ...
        <form action="{{ route('admin.about_goals1.store') }}" method="POST" @submit="loading = true">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 text-right">
                <div>
                    ...
                    <input type="text" name="title" x-model="form.title" placeholder="مثال: تنظيم العمل الخيري الرقمي"
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-400 mb-2 mr-1">الوصف التفصيلي</label>
                    <textarea name="description" x-model="form.description" rows="2" placeholder="مثال: تجميع المبادرات والحالات..."
                        class="w-full px-5 py-4 rounded-xl border @if($errors->has('description') && !session('edit_id')) border-rose-500 ring-4 ring-rose-500/10 @else border-slate-200 dark:border-slate-700 focus:ring-4 focus:ring-brand-500/10 @endif bg-slate-50 dark:bg-dark-900 outline-none transition-all font-medium text-sm leading-relaxed text-slate-600 dark:text-slate-300"></textarea>
    ...
    <div class="mb-10 w-full">
        <div class="mb-4 flex items-center justify-between">
            <h3 class="text-xs md:text-sm font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2">
                <i class="fa-regular fa-eye"></i> معاينة التصميم (اضغط على هدف من الجدول لعرضه)
            </h3>
        </div>

        <div class="bg-slate-50 dark:bg-dark-900 rounded-[2rem] p-8 md:p-16 border border-slate-200/50 dark:border-slate-700/50 flex items-center justify-center min-h-[300px] overflow-hidden">

            <div class="relative bg-white dark:bg-dark-800 border border-slate-200 dark:border-slate-700/80 rounded-xl p-8 md:p-10 w-full max-w-md text-center shadow-sm flex flex-col justify-center items-center min-h-[220px] transition-all duration-500 overflow-hidden"
                 :class="previewItem ? 'scale-100 opacity-100' : 'scale-95 opacity-90'">

                <span class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-[10rem] md:text-[14rem] font-black text-slate-100 dark:text-white/5 select-none z-0 pointer-events-none transition-all duration-300"
                      x-text="previewItem ? previewIndex : '1'"></span>

                <div class="relative z-10">
                    <h4 class="text-xl md:text-2xl font-bold text-slate-800 dark:text-white mb-4 leading-tight transition-colors"
                        x-text="previewItem ? previewItem.title : (form.title || 'عنوان الهدف')"></h4>

                    <p class="text-sm md:text-base text-slate-600 dark:text-slate-400 leading-relaxed font-medium transition-colors"
                       x-text="previewItem ? previewItem.description : (form.description || 'وصف تفصيلي للهدف يوضح الغاية منه وكيفية تحقيقه...')"></p>
                </div>
            </div>
    ...
    <div class="bg-white dark:bg-dark-800 rounded-3xl border border-slate-100 dark:border-slate-700/50 shadow-sm overflow-hidden w-full">
        ...
        <div class="p-6 border-b border-slate-100 dark:border-slate-700/50 bg-slate-50/50 dark:bg-dark-800 flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div class="w-full md:w-80 relative">
                <input type="text" x-model="search" placeholder="ابحث عن هدف أو وصف..."
                    ...
                    <tr>
                        <td colspan="4" class="px-6 py-16 text-center">
                            <div class="w-20 h-20 bg-slate-50 dark:bg-dark-900 rounded-full flex items-center justify-center mx-auto mb-4">
                                <i class="fa-solid fa-bullseye text-slate-300 dark:text-slate-600 text-3xl"></i>
                            </div>
        ...
        <div>
            <div x-show="editModal" x-cloak class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
                <div @click.away="!loading && (editModal = false)" class="bg-white dark:bg-dark-800 w-full max-w-lg rounded-3xl shadow-2xl p-8 border border-slate-100 dark:border-slate-700 transform transition-all">
                    ...
                    <div class="flex justify-between items-center mb-8 border-b border-slate-100 dark:border-slate-700/50 pb-4">
                        <h3 class="text-xl font-bold text-slate-800 dark:text-white">تعديل الهدف</h3>
                    ...
                    <form :action="'{{ url('admin/about-goals1') }}/' + itemToEdit.id" method="POST" @submit="loading = true">
                        @csrf @method('PUT')
            ...
            <div x-show="deleteModal" x-cloak class="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
                <div @click.away="!loading && (deleteModal = false)" class="bg-white dark:bg-dark-800 w-full max-w-sm rounded-3xl p-8 text-center shadow-2xl border border-slate-100 dark:border-slate-700/50 transform transition-all">
                    ...
                    <form :action="'{{ url('admin/about-goals1') }}/' + itemToDelete" method="POST" @submit="loading = true">
                        @csrf @method('DELETE')
