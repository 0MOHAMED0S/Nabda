...
@section('title', 'مسيرة المنصة (الخط الزمني)')
...
<div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8" x-data="{
    ...
    // State for Live Preview (Activated on Click)
    previewItem: null,

    init() {
        // Set first item as default preview if available
        @if(isset($histories) && $histories->count() > 0)
            this.previewItem = @js($histories->first());
        @endif

        // فتح نافذة التعديل تلقائياً إذا كان هناك خطأ
    ...
    <div class="bg-white dark:bg-dark-800 p-6 md:p-8 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700/50 mb-10 w-full">
        <div class="flex items-center gap-3 mb-8 border-b border-slate-100 dark:border-slate-700/50 pb-6">
            ...
            <h3 class="text-2xl font-bold text-slate-800 dark:text-white">إضافة محطة جديدة</h3>
        </div>

        <form action="{{ route('admin.about_histories.store') }}" method="POST" @submit="loading = true">
            @csrf

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 text-right">
                <div>
                    <label class="block text-xs font-bold text-slate-400 mb-2 mr-1">العنوان (التاريخ - الحدث)</label>
                    <input type="text" name="title" value="{{ !session('edit_id') ? old('title') : '' }}" placeholder="مثال: أكتوبر ٢٠٢٥ - بداية الفكرة"
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-400 mb-2 mr-1">التفاصيل والوصف</label>
                    <textarea name="description" rows="3" placeholder="مثال: ولدت فكرة نبضة خير من حاجة حقيقية..."
            ...
            <div class="flex justify-end pt-6 border-t border-slate-100 dark:border-slate-700/50">
                <button type="submit" :disabled="loading" class="w-full md:w-auto bg-brand-600 text-white px-10 py-4 rounded-xl font-bold flex items-center justify-center gap-3 disabled:opacity-70 disabled:cursor-not-allowed shadow-lg shadow-brand-500/20 hover:bg-brand-700 hover:-translate-y-0.5 transition-all text-base">
                    ...
                    <span x-text="loading ? 'جاري الإضافة...' : 'إضافة للخط الزمني'"></span>
                </button>
            </div>
        </form>
    </div>
    ...
    <div class="mb-10 w-full">
        <div class="mb-4 flex items-center justify-between">
            <h3 class="text-xs md:text-sm font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2">
                <i class="fa-regular fa-eye"></i> معاينة حية للتصميم (اضغط على محطة من الجدول لعرضها)
            </h3>
        </div>

        <div class="bg-white dark:bg-dark-800 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700/50 p-8 md:p-12 relative overflow-hidden min-h-[250px] flex items-center justify-center w-full">
            <div class="absolute top-0 right-0 w-full h-full bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-brand-500/10 via-transparent to-transparent -z-10 pointer-events-none"></div>

            <div class="relative w-full max-w-2xl mx-auto flex items-center transition-all duration-500"
                 :class="previewItem ? 'scale-100 opacity-100' : 'scale-95 opacity-50'">

                <div class="absolute right-1/2 top-1/2 -translate-y-1/2 w-[2px] h-40 bg-gradient-to-b from-transparent via-cyan-400 to-transparent shadow-[0_0_15px_rgba(34,211,238,0.8)] hidden md:block"></div>
                <div class="absolute right-1/2 top-1/2 translate-x-1/2 -translate-y-1/2 w-5 h-5 rounded-full bg-cyan-400 border-4 border-white dark:border-dark-800 shadow-[0_0_15px_rgba(34,211,238,0.8)] z-10 hidden md:block"></div>

                <div class="w-full md:w-[calc(50%-2.5rem)] mr-auto text-right bg-slate-100 dark:bg-dark-900 rounded-2xl p-8 shadow-sm border border-slate-200/50 dark:border-slate-700/50 relative">
                    <div class="absolute top-1/2 -right-3 -translate-y-1/2 w-0 h-0 border-t-[10px] border-t-transparent border-l-[12px] border-l-slate-100 dark:border-l-dark-900 border-b-[10px] border-b-transparent hidden md:block"></div>

                    <h3 class="text-xl md:text-2xl font-black text-slate-800 dark:text-white mb-3 transition-colors" x-text="previewItem ? previewItem.title : 'عنوان المحطة'"></h3>
                    <p class="text-sm text-slate-500 dark:text-slate-400 leading-relaxed font-medium transition-colors" x-text="previewItem ? previewItem.description : 'تفاصيل الحدث التاريخي...'"></p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-dark-800 rounded-3xl border border-slate-100 dark:border-slate-700/50 shadow-sm overflow-hidden w-full">
        ...
        <div class="p-6 border-b border-slate-100 dark:border-slate-700/50 bg-slate-50/50 dark:bg-dark-800 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <h3 class="text-lg font-bold text-slate-800 dark:text-white">قائمة المحطات الزمنية</h3>
            <div class="w-full md:w-80 relative">
                <input type="text" x-model="search" placeholder="ابحث عن محطة أو حدث..."
            ...
            <table class="w-full text-right border-collapse">
                <thead class="bg-slate-50 dark:bg-dark-900/50 border-b border-slate-100 dark:border-slate-700/50">
                    <tr>
                        ...
                        <th class="px-6 py-5 text-xs font-bold text-slate-400 uppercase w-1/3">المحطة / الحدث</th>
                        <th class="px-6 py-5 text-xs font-bold text-slate-400 uppercase">التفاصيل</th>
                    ...
                    <tr>
                        <td colspan="4" class="px-6 py-16 text-center">
                            <div class="w-20 h-20 bg-slate-50 dark:bg-dark-900 rounded-full flex items-center justify-center mx-auto mb-4">
                                <i class="fa-solid fa-clock-rotate-left text-slate-300 dark:text-slate-600 text-3xl"></i>
                            </div>
                            <p class="text-slate-500 font-medium text-lg">لم يتم إضافة أي محطات زمنية بعد.</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
    ...
    <template x-teleport="body">
        <div>
            <div x-show="editModal" x-cloak class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
                <div @click.away="!loading && (editModal = false)" class="bg-white dark:bg-dark-800 w-full max-w-lg rounded-3xl shadow-2xl p-8 border border-slate-100 dark:border-slate-700 transform transition-all">

                    <div class="flex justify-between items-center mb-8 border-b border-slate-100 dark:border-slate-700/50 pb-4">
                        <h3 class="text-xl font-bold text-slate-800 dark:text-white">تعديل المحطة</h3>
                    ...
                    <form :action="'{{ url('admin/about-histories') }}/' + itemToEdit.id" method="POST" @submit="loading = true">
                        @csrf @method('PUT')
                        ...
                        <div class="space-y-6 text-right">
                            <div>
                                ...
                                <input type="text" name="title" x-model="itemToEdit.title"
                                    :class="{
                                        'border-rose-500 ring-4 ring-rose-500/10': {{ $errors->has('title') ? 'true' : 'false' }} && '{{ session('edit_id') }}' == itemToEdit.id,
                                        'border-slate-200 dark:border-slate-700 focus:ring-4 focus:ring-brand-500/10': !({{ $errors->has('title') ? 'true' : 'false' }} && '{{ session('edit_id') }}' == itemToEdit.id)
                                    }"
                            ...
                            <div>
                                ...
                                <textarea name="description" rows="4" x-model="itemToEdit.description"
                                    :class="{
                                        'border-rose-500 ring-4 ring-rose-500/10': {{ $errors->has('description') ? 'true' : 'false' }} && '{{ session('edit_id') }}' == itemToEdit.id,
                                        'border-slate-200 dark:border-slate-700 focus:ring-4 focus:ring-brand-500/10': !({{ $errors->has('description') ? 'true' : 'false' }} && '{{ session('edit_id') }}' == itemToEdit.id)
                                    }"
            ...
            <div x-show="deleteModal" x-cloak class="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
                <div @click.away="!loading && (deleteModal = false)" class="bg-white dark:bg-dark-800 w-full max-w-sm rounded-3xl p-8 text-center shadow-2xl border border-slate-100 dark:border-slate-700/50 transform transition-all">
                    ...
                    <p class="text-sm text-slate-500 dark:text-slate-400 mb-8 leading-relaxed">هل أنت متأكد من رغبتك في حذف هذه المحطة التاريخية؟</p>

                    <form :action="'{{ url('admin/about-histories') }}/' + itemToDelete" method="POST" @submit="loading = true">
                        @csrf @method('DELETE')
