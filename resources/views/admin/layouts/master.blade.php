...
<body
    class="bg-[#f8fafc] dark:bg-dark-900 text-slate-900 dark:text-slate-100 min-h-screen transition-colors duration-300">
    ...
    <div x-data="{
        ...
        <div
            class="pointer-events-auto relative overflow-hidden bg-white dark:bg-dark-800 rounded-2xl shadow-2xl border border-slate-100 dark:border-slate-700/50 p-4 flex items-start gap-4">
            ...
            <div :class="type === 'success' ? 'bg-emerald-100 text-emerald-600' : 'bg-rose-100 text-rose-600'"
                ...
                <i class="fa-solid"
                    :class="type === 'success' ? 'fa-circle-check text-lg' : 'fa-circle-exclamation text-lg'"></i>
            </div>
    ...
    <div class="flex min-h-screen">
        ...
        <aside id="main-sidebar" :class="sidebarOpen ? 'translate-x-0' : 'translate-x-full lg:translate-x-0'"
            ...
            <div class="p-6 mb-4">
                <div class="flex items-center justify-between">
                    <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-3 group">
                        <img src="{{ asset('logo/logo.webp') }}" alt="نبضة خير"
                    ...
                    $navItems = [
                        ...
                        [
                            ...
                            'active_routes' => [
                                ...
                                'admin.about_visions.index',
                                'admin.about_goals1.index',
                                'admin.about_goals2.index',
                                'admin.about_histories.index',
                                'admin.team.index',
                            ],
                            'children' => [
                    ...
                    @if ($item['is_group'])
                        ...
                        <div x-data="{ open: {{ $isGroupActive ? 'true' : 'false' }} }" class="mb-1">
                            <button @click="open = !open"
                                :class="open ? 'bg-slate-50 dark:bg-dark-700/40 text-brand-600' :
                                    'text-slate-500 hover:bg-slate-50 dark:hover:bg-dark-700/30'"
                            ...
                            <div x-show="open" x-collapse>
                                <div
                                    class="mt-1 mr-4 pr-3 border-r-2 border-slate-100 dark:border-slate-700 flex flex-col gap-1 py-1">
                                    @foreach ($item['children'] as $child)
                                        @php $isChildActive = request()->routeIs($child['route']); @endphp
                                        <a href="{{ route($child['route']) }}"
                                            id="{{ $isChildActive ? 'active-link' : '' }}"
            ...
            <div class="p-4 border-t border-slate-100 dark:border-slate-700/50">
                <form action="{{ route('admin.logout') }}" method="POST">
                    @csrf
                    <button type="submit"
                        class="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-500/10 transition-all font-bold text-sm group">
        ...
        <main class="flex-1 flex flex-col min-w-0">
            <header
                class="h-20 bg-white/80 dark:bg-dark-800/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-700/50 px-4 sm:px-8 flex items-center justify-between sticky top-0 z-40">
                <div class="flex items-center gap-4">
                    <button @click="sidebarOpen = true" class="lg:hidden p-2 text-slate-500"><i
                            class="fa-solid fa-bars-staggered text-xl"></i></button>
                    <button @click="darkMode = !darkMode; localStorage.setItem('dark', darkMode)"
                        class="p-2.5 rounded-xl bg-slate-100 dark:bg-dark-700 text-slate-600 dark:text-amber-400 transition-all">
                ...
                <div class="flex items-center gap-4">
                    <a href="{{ route('admin.profile') }}" class="flex items-center gap-4 group cursor-pointer">
                        <div class="text-right hidden sm:block">
                            <p
                                class="text-sm font-bold text-slate-800 dark:text-white leading-none mb-1 group-hover:text-brand-600 transition-colors">
                                {{ auth('admin')->user()->name ?? 'مدير النظام' }}</p>
                            <span class="text-[10px] text-brand-600 font-bold uppercase tracking-tighter">إعدادات
                                الحساب</span>
                        </div>
